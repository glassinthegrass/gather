...
const Person = (props) => {
  ...
  let edit = toggle?<Delete onClick={()=>handleDelete(person_id,user_id)}>delete person</Delete>:<></>
  let groupDisplay = groupToggle? <GroupToggle onClick={()=>handleGroupToggle()}>Close</GroupToggle>:<GroupToggle onClick={()=>handleGroupToggle()}>Groups</GroupToggle>
  return (
    <Container>
      {edit}
    <PersonContainer>
    ...
    <PersonGroups handleGroupDelete={props.handleGroupDelete} push={props.push} person_id={person_id} groupToggle={groupToggle} />
    </Container>
  );
};
